...
designed to resist and the concrete defenses in the code. It is aimed at
the ops team deploying the server on TrueNAS SCALE, not at end users.
...
| T2 | **Volumetric DoS against the server itself** (UDP flood, SYN flood, slow reads) | Server availability, TrueNAS host stability |
| T3 | **Resource exhaustion inside the process** (unbounded maps, file descriptors) | Availability, memory pressure on the host |
| T4 | **Disk exhaustion via the session log** | Availability of the TrueNAS host's /var |
...
| T7 | **Port/service collision with the TrueNAS web UI** | Operator footgun; accidental lockout |
| T8 | **False positives for legitimate players** (our limits triggering before the ISP's) | Tool accuracy, customer trust |
| T9 | **Interference with the real Torch/SE server** on the same host | Production impact |

## Defenses

Each row below maps to real code, with file:line cites where useful.
...
Code: [server/a2sResponder.js](../server/a2sResponder.js),
[server/challengeTokens.js](../server/challengeTokens.js).
...
Code: [server/gameShape.js](../server/gameShape.js),
[server/challengeTokens.js](../server/challengeTokens.js).
...
Code: [server/server.js:buildRateLimited](../server/server.js).
...
Code: [server/rateLimiter.js](../server/rateLimiter.js) and
the `handleUdpMessage` / `handleTcpConnection` functions in
...
[client/client.js](../client/client.js)) distinguishes this from real
ISP packet loss in its result table so the limiter never causes a
...
Code: [server/sessionLog.js](../server/sessionLog.js),
[server/rateLimiter.js](../server/rateLimiter.js),
...
TrueNAS SCALE runs Docker containers on the host kernel. If the worst
...
Code: [server/docker-compose.yml](../server/docker-compose.yml),
[server/Dockerfile](../server/Dockerfile).

### T7 — Port collision with the TrueNAS web UI

The original design listened on TCP 80, TCP 443, and UDP 443 as
"baseline" controls. All three are impossible to bind on the TrueNAS
host network: TrueNAS SCALE's own web UI binds TCP 443, and binding
80/443 requires CAP_NET_BIND_SERVICE which we just dropped.
...
Code: [shared/ports.js](../shared/ports.js).
...
Code: [client/client.js](../client/client.js) — `udpProbe`,
`udpLossTest`, and the `printTable` function.

### T9 — Interference with the real Torch/SE server

If you deploy the test server on the same TrueNAS host that runs a
real SE dedicated server, **the two will conflict on UDP 27015 /
27016**. This is deliberate: the whole point of the test is to probe
the same ports, from the same IP, with the same shape of traffic. But
the conflict is real and operational.

**Recommended split:**
- **Separate host, separate public IP** is strongly preferred. Run the
  test server on any non-production TrueNAS box or a small VM. The
  customer still gets equivalent diagnostic data because ISP
  treatment of UDP 27016 is IP-agnostic at the DPI layer: if T-Mobile
  drops SE traffic to one public IP it drops it to all of them.
- **Same host, different IP** is acceptable if the TrueNAS host has
  multiple public addresses. Bind the test server to the second IP
  via `SDG_CT_BIND` and set NAT rules accordingly.
...
- **No edge rate limiting (iptables/ipset/nftables).** TrueNAS SCALE
  24.10 does not give app operators an ergonomic interface to host
  firewall rules from inside a custom app, and edge rules are easy to
  desync from the app. All rate limiting is in-process.
